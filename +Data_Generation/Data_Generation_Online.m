...
function [Xtraining_Array_RSRP, Ytraining_regression_double_RSRP, Xvalidation_RSRP, Yvalidation_regression_double_RSRP] = Data_Generation_Online(Training_set_ratio, SNR_Range, Num_of_frame_each_SNR, type)
...
elseif strcmp(type, 'Propogation')
    ...
    [Multitap_Channel_Signal, ~, Multitap_Channel_Signal_user_for_channel, ~] = Channel.Propagation_Channel_Model_Online(Transmitted_signal, Transmitted_signal_for_channel, SNR_OFDM, SampleRate, Carrier_Frequency, PathDelays, AveragePathGains, MaxDopplerShift, DelayProfile);
...
if Frame <= fix(Training_set_ratio * Num_of_frame_each_SNR)
    ...
    Ytraining_regression_double_RSRP(:, 1, 1, Training_index) = reshape(real(H_Ref(:, Pilot_location_symbols + 1)), [], 1);
    Ytraining_regression_double_RSRP(:, 2, 1, Training_index) = reshape(imag(H_Ref(:, Pilot_location_symbols + 1)), [], 1);
else
    ...
    Yvalidation_regression_double_RSRP(:, 1, 1, Validation_index) = reshape(real(H_Ref(:, Pilot_location_symbols + 1)), [], 1);
    Yvalidation_regression_double_RSRP(:, 2, 1, Validation_index) = reshape(imag(H_Ref(:, Pilot_location_symbols + 1)), [], 1);
end
