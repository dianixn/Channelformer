function [Fading_signal, Fading_signal_noise_free, Reference_Signal_channel, Reference_Signal] = Propagation_Channel_Model_test(Transmitted_signal, Transmitted_signal_for_channel, SNR, SampleRate, Carrier_Frequency, PathDelays, AveragePathGains, MaxDopplerShift, DelayProfile)
...
Nvariance = sqrt(Noise_Variance / 2);
n = Nvariance * (randn(length(Reference_Signal), 1) + 1j * randn(length(Reference_Signal), 1)); % Noise generation

Reference_Signal_channel = Reference_Signal + n;
