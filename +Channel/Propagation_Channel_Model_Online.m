function [Fading_signal, Fading_signal_noise_free, Reference_Signal_noise, Reference_Signal, h] = Propagation_Channel_Model_Online(Transmitted_signal, Transmitted_signal_for_channel, SNR, SampleRate, Carrier_Frequency, PathDelays, AveragePathGains, MaxDopplerShift, DelayProfile)
...
SignalPower_ref = mean(abs(Reference_Signal) .^ 2);
Noise_Variance_ref = SignalPower_ref / (10 ^ ( (SNR + 5) / 10)); %  

Nvariance_ref = sqrt(Noise_Variance_ref / 2);
n_ref = Nvariance_ref * (randn(length(Transmitted_signal), 1) + 1j * randn(length(Transmitted_signal), 1)); % Noise generation

Fading_signal = Fading_signal_noise_free + n;

Reference_Signal_noise = Reference_Signal + n_ref;
